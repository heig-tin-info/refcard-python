...
\section*{Modules essentiels}
\begin{description}
\item[os, sys] système d'exploitation et interpréteur
\item[pathlib] manipulation moderne de chemins
\item[json, pickle] sérialisation de données
\item[re] expressions régulières
\item[datetime] dates et heures
\item[collections] conteneurs spécialisés
\item[itertools] itérateurs avancés
\item[functools] outils pour fonctions
\item[typing] annotations de types
\item[dataclasses] classes de données
\item[asyncio] programmation asynchrone
\item[logging] journalisation
\item[unittest, pytest] tests unitaires
\item[requests] requêtes HTTP
\item[numpy] calcul numérique
\item[pandas] analyse de données
\item[matplotlib] graphiques
\end{description}
